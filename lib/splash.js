...
export default function splash({ name, org, logo, active, total, channels, populate, iframe }){
  if (populate && populate.email) {
    var {email} = populate
    if (populate.first_name && populate.last_name)
      var {first_name,last_name} = populate
  }
  ...
  let div = dom('.splash',
    ...
    dom('p',
      ...
      ' on Slack to instant chat with your pair.'
    ),
    dom('form',
      ...
      dom('input.form-item name=email type=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')
        + (email ? ` value=${email}` : '')),
      dom('input type=hidden name=first_name'
        + (first_name ? ` value=${first_name}` : '')),
      dom('input type=hidden name=last_name'
