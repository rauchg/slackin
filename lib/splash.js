...
export default function splash({ path, name, org, coc, logo, active, total, channels, iframe, segment }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    dom('form id=slack-form',
      ...
      dom('input.form-item id=email type=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      coc && dom('.coc',
        dom('label',
    ...
    dom('script src=https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js'),
    dom('script src=https://cdn.crate.io/libs/jquery-cookie/1.4.1/jquery.cookie.min.js'),
    dom('script', `
      var segment_key = "${segment}";
    `),
    dom(`script src=${path}assets/crate.js`)
  );
  return div;
}
