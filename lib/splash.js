...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p',
      'Λάβετε τώρα μία πρόσκληση για την ', dom('b', name),
      // mention single single-channel inline
      channels && channels.length === 1 && dom('span', ' #', channels[0]),
      ' στο Slack και ελάτε σε επαφή με έλληνες επαγγελματίες του χώρου από όλο τον κόσμο.'
    ),
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' χρήστες online από τους ',
          dom('b.total', total), ' εγγεγραμμένους.'
        ]
        : [dom('b.total', total), ' χρήστες έχουν κάνει εγγραφή ως τώρα.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      coc && dom('.coc',
        dom('label',
          ...
          'Συμφωνώ με τους ',
          dom('a', { href: coc, target: '_blank' }, 'όρους χρήσης'),
          '.'
        )
      ),
      dom('button.loading', 'Στειλτε μου μια προσκληση')
    ),
    dom('p.signin',
      'ή ',
      dom(`a href=https://${org}.slack.com target=_top`, 'κάντε είσοδο'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
