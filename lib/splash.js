...
export default function splash({ path, name, org, coc, logo, active, total, channels, iframe, localization }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ),
    localization.join(name, channels),
    localization.state(active, total),
    dom('form',
      ...
      dom('input.form-item type=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      coc && localization.coc(coc),
      localization.getinvite()
    ),
    !iframe && localization.signin(org),
    !iframe && dom('footer',
      localization.powered,
