...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('form id=invite',
      ...
      dom('input.form-item name=first_name placeholder="First Name" '
        + (!iframe ? 'autofocus' : '')),
      dom('input.form-item name=last_name placeholder="Last Name" '),
      dom('input.form-item name=source placeholder="How Did You Hear About StackStorm?" '),
