  ...
  .end(function (err, res){
    ...
    if (!ok) {
      if (providedError === 'missing_scope' && needed === 'admin') {
        ...
        fn(new Error('Vous avez déjà été invié à Slack. Assurez-vous d\'avoir reçu un courriel de feedback@slack.com.'))
      } else if (providedError === 'already_in_team') {
        fn(new Error(`Nous vous redirigeons vers Slack...`))
      } else {
