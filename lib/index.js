  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'L\'accès à cette chaîne ne vous est pas autorisé.' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Chaîne non trouvée "${channel}"` })
      }
    }
    ...
    if (!email) {
      return res
      .status(400)
      .json({ msg: 'Courriel manquant' })
    }

    if(captcha_response == undefined || !captcha_response.length){
      return res
      .status(400)
      .send({ msg: 'Captcha invalide' });
    }

    if (!remail().test(email)) {
      return res
      .status(400)
      .json({ msg: 'Courriel invalide' })
    }
    ...
    if (emails && emails.indexOf(email) === -1) {
      return res
      .status(400)
      .json({ msg: 'Votre courriel n\'est pas sur la liste des courriels autorisés' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'Vous devez accepter les conditions d\'utilisation' })
    }
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          invite({ token, org, email, channel: chanId }, err => {
            if (err) {
              if (err.message === `Nous vous redirigeons vers Slack...`) {
                return res
                .status(303)
            ...
            res
            .status(200)
            .json({ msg: 'Parfait. Consultez votre boite courriel !' })
          });
          ...
          if (err) {
            return res
            .status(400)
            .send({ msg: "Vérification captcha échouée" });
          }
        }
