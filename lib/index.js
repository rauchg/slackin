  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'Μη επιτρεπτό κανάλι' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Το κανάλι "${channel}" δεν βρέθηκε` })
      }
    }
    ...
    if(captcha_response == undefined || !captcha_response.length){
      return res
      .status(400)
      .send({ msg: 'Λάθος captcha' });
    }

    if (!remail().test(email)) {
      return res
      .status(400)
      .json({ msg: 'Λανθασμένο email' })
    }
    ...
    if (emails && emails.indexOf(email) === -1) {
      return res
      .status(400)
      .json({ msg: 'Το email σας δεν βρίσκεται στην λίστα αποδεκτών' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'Πρέπει να συμφωνήσετε με τους όρους χρήσης' })
    }
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          invite({ token, org, email, channel: chanId }, err => {
            if (err) {
              if (err.message === `Σας στέλνουμε στο Slack...`) {
                return res
                .status(303)
            ...
            res
            .status(200)
            .json({ msg: 'Παρακαλούμε ελέξτε το email σας!' })
          });
          ...
          if (err) {
            return res
            .status(400)
            .send({ msg: "Ο έλεγχος captcha απέτυχε" });
          }
        }
