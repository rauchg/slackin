...
import { invite, verifyRecaptcha } from './slack-invite'
...
export default function slackin ({
  ...
  if(!recaptchaSecret) throw new Error('Must provide recaptcha secret')
  ...
  app.post('/invite', json(), (req, res, next) => {
    verifyRecaptcha(recaptchaSecret, req.body.recaptcha, err => {
        if (err) {
        return res
          .status(400)
