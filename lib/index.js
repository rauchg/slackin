...
export default function slackin({
  ...
  stylesheet,
  channels,
  muted = false // jshint ignore:line
}){
  // must haves
  ...
  app.get('/', (req, res) => {
    ...
    let { email, f, l } = req.query
    var populate = null
    if (email) populate = { email, first_name:f, last_name:l }
    ...
    let page = dom('html',
      dom('head',
        ...
        splash({ stylesheet, name, org, logo, channels, active, populate, total }), 
        stylesheet && dom('link rel=stylesheet', { href: stylesheet }),  // TODO, read via GET on app load and thrown on page to stop flicker
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    invite({ token, org, invite:{email,first_name,last_name}, channel: chanId }, err => {
      if (err) {
        return res
        .status(400)
