...
body.addEventListener('submit', function (ev){
  ...
  invite(channel ? channel.value : null, coc && coc.checked ? 1 : 0, first_name.value, last_name.value, source.value,
      email.value, function (err, msg){
    if (err) {
...
function invite (channel, coc, first_name, last_name, source, email, fn){
  request
