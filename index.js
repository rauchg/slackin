...
export default function slackin({
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    var ipArray = ['97.81.33.244', '50.205.178.174'];
    ...
    // Simple IP-based blocking, this may totally break all the things
    if (ipArray.indexOf(ip) > -1) {
      console.log("Blocked invite from ${ip}")
      return res
      .status(400)
      .json({ msg: 'Agreement to CoC is mandatory' });
      .json({ msg: 'Unable to process invite' });
    }
